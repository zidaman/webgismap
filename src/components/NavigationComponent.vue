<template>
    <div class="navigateComponent-pannel" v-show="this.$store.getters._getDefaultNavigateVisible">
        <div class="navigateComponent-header">
            <span>行政区划导航</span>
            <i class="el-icon-close" @click="closeNavigatePannel"></i>
        </div>
        <div class="navigate-select-pannel">
            <span class="navigate-select-label">省份：</span>
            <el-select
        ...
        <div class="navigate-content-pannel">
            <tbody>
    ...
    methods: {
                ...
                Promise.all(
                    currentCityData.map(async (item2) => {
                        ...
                        const queryTask2 = new QueryTask({
                            url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/County_WebMercator/FeatureServer/0',
                        });
        ...
        async handleItemClick(val, type) {
            ...
            if (type === 'city') {
                ...
                serverUrl =
                    'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/City_WebMercator/FeatureServer/0';
            } else if (type === 'county') {
...
.navigateComponent-header > span {
    line-height: 35px;
    font-size: 16px;
    font-weight: 600;
}
.navigateComponent-header > i {
    line-height: 35px;
    cursor: pointer;
}
.navigate-select-pannel {
    width: 100%;
    height: 30px;
    padding: 0 5px;
    box-sizing: border-box;
